import { Component } from "react"
export enum RecordingItemStatus {
    uploading, processing, done, error
}
type RecordingCardStatusTag_props = {
    status:RecordingItemStatus
}
type RecordingCardStatusTag_state = {
}
/**
 * 
 */
export class RecordingCardStatusTag extends Component<RecordingCardStatusTag_props, RecordingCardStatusTag_state>{
    constructor(props:Readonly<RecordingCardStatusTag_props>) {
        super(props)
        this.state = {
        }
    }
    render() {
        return (
            <>
           
                {this.props.status== RecordingItemStatus.uploading&&
                (<span className="tag icon green">
                  <div className="icon">

                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="bi bi-clock" viewBox="0 0 16 16">
                      <path d="M8 3.5a.5.5 0 0 0-1 0V9a.5.5 0 0 0 .252.434l3.5 2a.5.5 0 0 0 .496-.868L8 8.71z" />
                      <path d="M8 16A8 8 0 1 0 8 0a8 8 0 0 0 0 16m7-8A7 7 0 1 1 1 8a7 7 0 0 1 14 0" />
                    </svg>
                  </div>
                  <span className="content">uploading</span>
                </span>)}
                
               { this.props.status== RecordingItemStatus.processing&&
                <span className="tag icon blue">
                  <div className="icon spinner">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="bi bi-gear-fill" viewBox="0 0 16 16">
                      <path d="M9.405 1.05c-.413-1.4-2.397-1.4-2.81 0l-.1.34a1.464 1.464 0 0 1-2.105.872l-.31-.17c-1.283-.698-2.686.705-1.987 1.987l.169.311c.446.82.023 1.841-.872 2.105l-.34.1c-1.4.413-1.4 2.397 0 2.81l.34.1a1.464 1.464 0 0 1 .872 2.105l-.17.31c-.698 1.283.705 2.686 1.987 1.987l.311-.169a1.464 1.464 0 0 1 2.105.872l.1.34c.413 1.4 2.397 1.4 2.81 0l.1-.34a1.464 1.464 0 0 1 2.105-.872l.31.17c1.283.698 2.686-.705 1.987-1.987l-.169-.311a1.464 1.464 0 0 1 .872-2.105l.34-.1c1.4-.413 1.4-2.397 0-2.81l-.34-.1a1.464 1.464 0 0 1-.872-2.105l.17-.31c.698-1.283-.705-2.686-1.987-1.987l-.311.169a1.464 1.464 0 0 1-2.105-.872zM8 10.93a2.929 2.929 0 1 1 0-5.86 2.929 2.929 0 0 1 0 5.858z" />
                    </svg>
                  </div>
                  <span className="content">processing</span>
                </span>}
                
                {this.props.status== RecordingItemStatus.done&&
                <span className="tag icon">
                  <div className="icon">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-check-all" viewBox="0 0 16 16">
                      <path d="M8.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L2.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093L8.95 4.992zm-.92 5.14.92.92a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 1 0-1.091-1.028L9.477 9.417l-.485-.486z" />
                    </svg>
                  </div>
                  <span className="content">done</span>
                </span>}
                
                {(this.props.status== RecordingItemStatus.error)&&
                <div></div>}

    </>
            

        )
    }
}